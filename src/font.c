...
void font_render(float x, float y, float h, char* text) {
    ...
    while(*text) {
        if(*text>31 || font_type==FONT_SMALLFNT) {
            float tx = ((*text-((font_type==FONT_KNUMB)?32:0))%chars_x)/((float)chars_x);
            float ty = ((*text-((font_type==FONT_KNUMB)?32:0))/chars_x)/((float)chars_y);
    ...
    switch(font_type) {
        case FONT_SMALLFNT:
            ...
            break;
        case FONT_KNUMB:
            glBindTexture(GL_TEXTURE_2D,font_knumb.texture_id);
            break;
    }
